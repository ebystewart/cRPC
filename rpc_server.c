...
main(int argc, char **argv){
	...
	init_serialized_buffer_of_defined_size(&recv_b, 1024);
...
READ:
	len = recvfrom(sock_udp_fd, (char *)recv_b->buff, 1024, 0,(struct sockaddr *)&client_addr, &addr_len);

	out_b = rpc_server_process_msg(recv_b);

	len = sendto(sock_udp_fd, (char *)(out_b->buff), get_serialize_buffer_size(out_b), 0,
